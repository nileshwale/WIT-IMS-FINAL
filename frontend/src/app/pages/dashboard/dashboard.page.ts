  ...
  ionViewWillEnter() {
    ...
    this.http.get<any[]>('http://localhost:3000/api/items', { headers }).subscribe({
      next: async (res) => {
        ...
        const lowStockItems = this.items.filter(item => item.Quantity < this.lowStockThreshold);
        if (lowStockItems.length > 0) {
          const names = lowStockItems.map(i => i.ItemName).join(', ');
          const toast = await this.toastController.create({
            message: `Low stock alert for: ${names}`,
            duration: 4000,
            color: 'danger',
          });
          await toast.present();
